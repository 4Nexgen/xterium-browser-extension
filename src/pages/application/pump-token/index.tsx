...
const IndexPumpToken = () => {
  ...
  const getPumpTokens = () => {
    ...
    pumpTokenService
      ...
      .then((data) => {
        setPumpTokens(data)
        const tokensWithNetworkId = data.filter((token) => token.network_id)
        if (tokensWithNetworkId.length > 0) {
          setLoadingAssets(true)
          Promise.all(
            tokensWithNetworkId.map((token) => fetchAssetDetails(token.network_id))
          ).then(() => {
              ...
              filteredTokens.map((token) => (
                <div
                  ...
                  <h3 className="font-bold text-sm pl-2">
                    {token.assetDetail?.name || "No Name Available"} (
